...
A Streamlit dashboard makes the cost tradeoffs interactive and accessible to a non-technical audience.
...
An interactive Streamlit dashboard surfaces the Phase 2 business findings for a non-technical audience. Controls (model selector, classification threshold, investigation cost assumption) live in a persistent sidebar so they are always visible regardless of scroll position.
...
**Operational Reality** — the selected model and threshold are translated into transaction-level terms scaled to 1,000,000 transactions: how many flagged, how many genuine fraud caught, false alarm cost, fraud loss prevented, and net saving vs no detection.
...
assets/
    dashboard_overview.png          Screenshot: header, bottom line, threshold explorer
    dashboard_model_comparison.png  Screenshot: model comparison table
    dashboard_sensitivity.png       Screenshot: sensitivity analysis + operational reality
...
models/
    dashboard_data.json      Full validation sweep + test results for the dashboard
                             (generated by running cost_fraud_analysis.py)
...
# Phase 2 — cost-sensitive analysis (also generates models/dashboard_data.json)
...
Both analysis scripts resolve all paths relative to their own location and can be
...
The dashboard requires `models/dashboard_data.json`, which is generated by running
the Phase 2 script. The `.pkl` model files are not required to run the dashboard.

---

## Technologies

Python · pandas · scikit-learn · XGBoost · imbalanced-learn · Plotly · Streamlit · joblib
